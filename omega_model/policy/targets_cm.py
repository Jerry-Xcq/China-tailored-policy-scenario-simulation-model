...
class VehicleTargets(OMEGABase, VehicleTargetsBase):
    ...
    **Implements vehicle footprint-based GHG targets (CO2e g/mi).**
    This is a China-specific implementation where the attribute is
    vehicle curb mass (kg) instead of footprint (ft^2).
    ...
    def calc_target_co2e_gpmi(vehicle):
        """
        Calculate vehicle target CO2e g/mi based on curb mass (kg) and model year.
        ...
        if cache_key not in VehicleTargets._data:
            ...
            if len(start_years[start_years <= vehicle.model_year]) > 0:
                ...
                raise Exception('Missing curb-mass-based GHG CO2e g/mi target parameters for %s, %d or prior'
    ...
    def calc_cert_lifetime_vmt(reg_class_id, model_year):
        """
        Get lifetime VMT as a function of regulatory class and model year. 获取车辆的生命周期行驶里程（VMT）
