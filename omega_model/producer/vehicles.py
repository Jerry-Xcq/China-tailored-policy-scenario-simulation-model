...
class DecompositionAttributes(OMEGABase):
    ...
    **Stores vehicle-specific attributes that will be tracked at each point of a vehicle or composite vehicle frontier**
    ...
    def interp1d(vehicle, cost_curve, index_column, index_value, attribute_name):
        ...
        这个函数是为数值型数据设计的插值工具。它的核心任务是：根据一个给定的x轴值，从一条曲线上查出对应的y轴值。如果x值正好落在曲线的两个已知点之间，它会进行线性插值来计算y值
        Args:
        ...
        if cn in cost_curve:
            if len(cost_curve) > 1:#如果成本曲线不止一个点，说明可以进行插值
                if isinstance(index_value, Iterable):#如果是可以迭代对象，例如列表或者数组
                    # true interpolation (multiple values passed as index):则使用 np.interp 进行线性插值
                    return np.interp(index_value, cost_curve[index_column].values, cost_curve[cn].values)
                else:
                    # nearest result retrieval (get final result, single value):如果 index_value 是单个数值，则查找成本曲线上最接近该索引值的行，并返回该行的属性值
                    return cost_curve[cn].loc[abs(index_value - cost_curve[index_column]).idxmin()]#计算差值的绝对值，找到差值最小的那个点的索引，根据索引取出对应的值
            else:
    ...
    def interp1d_non_numeric(vehicle, cost_curve_non_numeric_data, index_column, index_value, attribute_name):
        """
        Interpolate non-numeric data这个函数是为非数值型数据（如字符串）设计的。由于字符串不能进行线性插值，它采用了一种**“最近邻”或“四舍五入”**的策略
...
class VehicleOnroadCalculations(OMEGABase):
    ...
    **Performs vehicle attribute calculations, as outlined in the input file.**
    ...
    def perform_onroad_calculations(vehicle, cost_cloud=None):
        ...
        Returns:根据预设规则，自动批量地对车辆或数据表中的属性进行条件判断和数学运算。 这样可以灵活地扩展和修改车辆属性的计算逻辑，无需硬编码每一种情况。
...
class CompositeVehicle(OMEGABase):
    ...
    **Implements vehicle composition and decomposition.**
    ...
    def __init__(self, vehicles_list, vehicle_id, calc_composite_cost_curve=True,
        ...
        for v in self.vehicle_list:#计算每辆车在组合中的份额
    ...
    def decompose(self):
        """执行完 cv.decompose() 后，构成这个复合车辆的每一个具体车型的最终生产计划（销量、技术、成本、排放等）都已经被完全确定了
        ...
        of composite ``cert_co2e_grams_per_mile`` and assigned the Vehicles.将一个宏观的、已经确定了技术水平和总销量的复合车辆，拆解回其内部的各个源车辆（source vehicles），并精确计算出每一个源车辆的具体属性
        ...
        for v in self.vehicle_list:
                ...
                for ccv in DecompositionAttributes.values:
                    v.__setattr__(ccv,
                              DecompositionAttributes.interp1d(v, self.cost_curve, cost_curve_interp_key,
                                                               ...
                                                               ccv)) #self.__getattribute__(cost_curve_interp_key)这个是复合车辆上该属性对应的值
            ...
            v.initial_registered_count = self.initial_registered_count * v.composite_vehicle_share_frac #每辆车占总体的比例在复合车辆的init中有计算
            v.set_target_co2e_Mg()  # varies by model year and initial_registered_count因车型年份和初始注册数量而异
        ...
        # calc weighted values
        for wv in self.weighted_values:#遍历所有需要加权计算的属性
            self.__setattr__(wv, weighted_value(self.vehicle_list, self.weight_by, wv))
    ...
    def calc_composite_cost_curve(self, plot=False):
        ...
        for v in self.vehicle_list:
            ...
            for wv in self.weighted_values:#新组合点的属性 = 之前组合的属性 × 之前份额 + 当前车辆属性 × 当前份额，再除以总份额
            ...
            # update running total market share更新总市场份额
            ...
            drop_columns = [c for c in composite_frontier_df.columns if c.endswith('_y') or c.endswith('_x') or
                            c.endswith('_market_share')] + ['_']#由于笛卡尔积和加权平均后，DataFrame 里会有很多临时列（如 `_x`, `_y`），这里先记录下来准备后续删除
            ...
            if plot:#每辆车一条线，复合车辆一条线；都是折线图（点之间有连线），不是纯散点图；用于比较不同车辆及其组合的成本曲线
                if v.name in omega_globals.options.plot_and_log_vehicles:# 重点车辆用黑色方块实线绘制
    ...
    def get_from_cost_curve(self, attribute_name, query_points):
        ...
        return DecompositionAttributes.interp1d(self, self.cost_curve, cost_curve_interp_key, query_points,
                                                attribute_name)#根据车辆的成本曲线数据，对指定属性在给定点上进行插值，返回插值结果
...
class Vehicle(OMEGABase):
    ...
    **Implements "candidate" or "working" vehicles for use during the producer compliance search.**
    ...
    mandatory_input_template_columns = {'vehicle_name', 'manufacturer_id', 'model_year', 'reg_class_id',
                                        ...
                                        'tractive_motor_kw', 'total_emachine_kw', 'nev_credit_per_vehicle'}
    ...
    def __init__(self, manufacturer_id):
        ...
        self.nev_credit_per_vehicle = 0.0  #: externally supplied NEV credit (credits/vehicle); 0 => trad vehicle
    ...
    def calc_cert_and_onroad_values(self, cloud):
        ...
        if self.fueling_class != 'ICE':#这里传入的是 drive_cycle_weight_year，它通常代表实际道路（onroad）条件下的工况权重年份，用于反映车辆在真实道路上的能耗表现
        ...
        if self.fueling_class != 'ICE':#这里传入的是 self.model_year，代表认证（certification）时的标准工况和年份，用于计算车辆在认证工况下的能耗
    ...
    def create_vehicle_clone(vehicle):
        ...
        alt_veh.in_production = is_up_for_redesign(alt_veh)#判断是否重新设计
    ...
    def clone_vehicle(vehicle):
        ...
        制作车辆的“克隆”，用于在基准年车队中创建车辆的替代动力总成版本
        Args:它会创建一个新的 Vehicle 对象，并继承原始车辆 v 的所有非动力总成相关的属性。这包括车身样式、尺寸、整备质量、底盘结构、安全配置等。你可以把它理解为复制了一个不带发动机和电池的“车壳+底盘”（即滑车身 Glider）
            vehicle (Vehicle): the vehicle to clone
        ...
        inherit_properties = ['name', 'compliance_id',
                              ...
                              'base_year_cost_curve_class', 'nev_credit_per_vehicle',
                              ] \
                              + Vehicle.dynamic_attributes
        #继承的属性里面没有base_year_product
    ...
    def init_vehicles_from_dataframe(df, verbose=False):
        ...
        for i in df.index:
            ...
            # 处理 curbweight_lbs 空值：如果为空或 NaN，使用上一个车辆的值
            '''curbweight_value = df.loc[i, 'curbweight_lbs']
            if pd.isna(curbweight_value) or curbweight_value == '' or curbweight_value == 0:
                # 使用上一个有效值，如果没有则使用默认值 3500 lbs（约 1588 kg）
                if i > 0 and hasattr(Vehicle, '_last_valid_curbweight_lbs'):
                    curbweight_value = Vehicle._last_valid_curbweight_lbs
                else:
                    curbweight_value = 3500  # 默认值
                omega_log.logwrite(f"Warning: curbweight_lbs is empty for vehicle '{df.loc[i, 'vehicle_name']}', using {curbweight_value} lbs")
            else:
                Vehicle._last_valid_curbweight_lbs = curbweight_value
            veh.curbweight_lbs = curbweight_value'''
            ...
            #veh.base_year_curbweight_lbs = veh.curbweight_lbs   使用已处理过空值的 curbweight_lbs
            ...
            veh.base_year_product = 1 #这是第一次读取数据，所以都要设定为基础年产品
            veh.base_year_gvwr_lbs = df.loc[i, 'gvwr_lbs']
            veh.base_year_gcwr_lbs = df.loc[i, 'gcwr_lbs']
            if 'nev_credit_per_vehicle' in df.columns:
                value = df.loc[i, 'nev_credit_per_vehicle']
                try:
                    value = float(value)
                ...
                if math.isnan(value):
                    value = 0.0
                veh.nev_credit_per_vehicle = value
            else:
                veh.nev_credit_per_vehicle = 0.0
        ...
        for v in vehicles_list:
            v.base_year_market_share = v.initial_registered_count / vehicle_shares_dict['total']# 1. 计算基准年市场份额
            # 2. 检查是否需要创建替代动力车型
            if v.fueling_class != 'BEV' or omega_globals.options.allow_ice_of_bev:
                if v.fueling_class == 'ICE':
                    # create BEV of ICE
                    alt_veh = Vehicle.create_vehicle_clone(v)# 首先，克隆一个车辆对象，继承原车非动力总成的属性（如车身、尺寸、底盘等）
                    ...
                    alt_veh.total_emachine_kw = 0  # motor size determined during cost curve generation设置燃料为电，重置发动机参数，电池和电机参数留待后续成本曲线生成时确定
                    ...
                    # BEV of ICE: 设置 sentinel 值 100，表示"待计算"，在 Pass1 结算前会被实际值覆盖
                    alt_veh.nev_credit_per_vehicle = 100
                    Vehicle.assign_vehicle_market_class_ID(alt_veh)
                    omega_globals.finalized_vehicles.append(alt_veh)# 将这个新的BEV车型添加到全局的最终车辆列表中
                    ...
                    alt_veh.battery_kwh = 0  # pack sizes determined by range target
                    alt_veh.total_emachine_kw = 0  # motor size determined during cost curve generation保留原发动机参数，但电池和电机参数留待后续确定
                    alt_veh.tractive_motor_kw = 0
                    alt_veh.onroad_charge_depleting_range_mi = 0  # determined during cost curve generation
                    ...
                    # PHEV of ICE: 直接赋值 NEV 积分 0.5
                    alt_veh.nev_credit_per_vehicle = 0.5
                    Vehicle.assign_vehicle_market_class_ID(alt_veh)
                    ...
                    alt_veh.in_use_fuel_id = "{'pump gasoline':1.0}"
                    alt_veh.cert_fuel_id = 'gasoline'
                    alt_veh.base_year_eng_rated_hp = v.total_emachine_kw / 0.746#根据原BEV的电机功率估算一个发动机功率，重置电池和电机参数
                    ...
                    # ICE of BEV: 传统燃油车无 NEV 积分
                    alt_veh.nev_credit_per_vehicle = 0
                    Vehicle.assign_vehicle_market_class_ID(alt_veh)
                    omega_globals.finalized_vehicles.append(alt_veh)
    ...
    def init_from_file(vehicle_onroad_calculations_file, verbose=False):
        ...
        Vehicle.init_vehicles_from_dataframe(omega_globals.options.vehicles_df, verbose=verbose) #这个的车辆df是上一个init中的 omega_globals.options.vehicles_df = agg_df ，是vehicle.csv 读取的
