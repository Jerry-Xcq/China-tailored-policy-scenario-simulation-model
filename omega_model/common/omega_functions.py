...
def calc_frontier(cloud, x_key, y_key, allow_upslope=False, invert_x_axis=True):
    ...
    # drop non-numeric columns so dtypes don't become "object" 删除非数值型列
    ...
    if len(cloud) > 1:
        ...
        if invert_x_axis:#根据是否反转 x 轴，设置 x 轴的符号
            x_sign = -1
        else:
            x_sign = 1

        # normalize data (helps with up-slope frontier) 对 x、y 数据进行归一化处理（0~1），便于后续计算
        ...
        # find frontier starting point, lowest x-value, and add to frontier 找到 x 最小的点，作为前沿的起点，加入前沿点列表
        ...
        if min(cloud[x_key].values) != max(cloud[x_key].values):
            while pd.notna(idxmin) and (min_frontier_factor <= 0 or allow_upslope) \
                ...
                prior_x = frontier_pts[-1][x_key] #取当前前沿点的 x、y 值，作为基准点
                ...
                cloud = cull_cloud(cloud, prior_x, x_key) #剔除所有 x 小于等于当前前沿点的点，只保留右侧的点

                '''这是原始的代码，会报错赋值问题，因为修改了def calc_frontier_factor_down函数（内部变成对cloud副本的修改），所以这里连带修改
                if not cloud.empty: #计算所有剩余点相对于当前前沿点的“frontier_factor”（通常是斜率或加权斜率）
                    calc_frontier_factor_down(cloud, prior_x, prior_y, x_key, y_key)
                    min_frontier_factor = min(cloud['frontier_factor'].values)#取最小的 frontier_factor
                ...
                '''这里修改后的过程是：
                首先 calc_frontier_factor_down 函数接收原始的 cloud DataFrame
                在 calc_frontier_factor_down 函数内部，通过 cloud = cloud.copy() 创建了一个新的 DataFrame 副本
                这个副本被修改（添加了 'frontier_factor' 列）
                修改后的副本被 calc_frontier_factor_down 函数返回
                在 calc_frontier 函数中，这个返回的 DataFrame 被重新赋值给 cloud 变量
                这样 cloud 变量现在引用的是包含 'frontier_factor' 列的新 DataFrame'''
                
                if not cloud.empty: #计算所有剩余点相对于当前前沿点的“frontier_factor”（通常是斜率或加权斜率）
                    cloud = calc_frontier_factor_down(cloud, prior_x, prior_y, x_key, y_key)
                    min_frontier_factor = min(cloud['frontier_factor'].values)#取最小的 frontier_factor

                    if min_frontier_factor > 0 and allow_upslope:
                        cloud = calc_frontier_factor_up(cloud, prior_x, prior_y, x_key, y_key)
                        min_frontier_factor = min(cloud['frontier_factor'].values)
...
def calc_frontier_factor_up(cloud, prior_x, prior_y, x_key, y_key):
    ...
    '''frontier_factor_values = ((prior_x - cloud[x_key].values) / 
                             (cloud[y_key].values - prior_y) ** 
                             omega_globals.options.cost_curve_frontier_affinity_factor)
    cloud["frontier_factor"] = frontier_factor_values'''

    # 确保我们在操作一个确定的副本
    '''cloud_copy = cloud.copy()   
    # frontier factor is different for up-slope (swap x & y and invert "y")
    frontier_factor_values = (prior_x - cloud_copy[x_key].values) / (cloud_copy[y_key].values - prior_y) \
                            ** omega_globals.options.cost_curve_frontier_affinity_factor    
    # 使用明确的副本赋值
    cloud_copy['frontier_factor'] = frontier_factor_values    
    # 将结果复制回原数据框
    cloud['frontier_factor'] = None
    cloud['frontier_factor'] = cloud_copy['frontier_factor'].values'''
    '''for i in range(len(cloud)):
        cloud.iat[i, cloud.columns.get_loc('frontier_factor')] = cloud_copy['frontier_factor'].iat[i]'''
...
def calc_frontier_factor_down(cloud, prior_x, prior_y, x_key, y_key):
    ...
    '''frontier_factor_values = (cloud[y_key].values - prior_y) / (cloud[x_key].values - prior_x) \
                               ** omega_globals.options.cost_curve_frontier_affinity_factor
    cloud.loc[:, "frontier_factor"] = frontier_factor_values'''
    
    # 确保我们在操作一个确定的副本
    '''cloud_copy = cloud.copy()
    # 计算 frontier factor 值
    frontier_factor_values = (cloud_copy[y_key].values - prior_y) / (cloud_copy[x_key].values - prior_x) \
                            ** omega_globals.options.cost_curve_frontier_affinity_factor
    # 使用明确的副本赋值
    cloud_copy['frontier_factor'] = frontier_factor_values
    # 将结果复制回原数据框
    ...
    '''frontier_factor_values = ((cloud[y_key].values - prior_y) / 
                             (cloud[x_key].values - prior_x) ** 
                             omega_globals.options.cost_curve_frontier_affinity_factor)
    cloud["frontier_factor"] = frontier_factor_values'''
...
def partition(column_names, num_levels=5, min_constraints=None, max_constraints=None, verbose=False):
    ...
    given constraints
    简单来说，如果你告诉它：“我有 A、B、C 三类产品，它们的总份额必须是100%。其中A最少占10%，B最多占50%。” 这个函数就会帮你列出所有满足这些条件的A、B、C的份额组合
    Args:
    ...
    if cache_key not in partition_dict:
        # --- 2. 输入参数初始化和规范化 ---
        if type(column_names) is int:# 如果 column_names 是一个整数 n，则生成一个从 0 到 n-1 的列表作为列名。
            column_names = [i for i in range(column_names)]

        min_level_dict = dict()# 初始化最小/最大约束字典。
        if min_constraints is None:
            min_constraints = dict()
        # 如果 min_constraints 是一个数字，则为所有列应用这个统一的最小约束。
        ...
        # --- 3. 填充初始约束值 ---
        # initialize min and max level dicts with nominal values遍历所有列，从传入的约束字典中填充 min/max_level_dict。
        for c in column_names:
            ...
            if c in max_constraints:
                ...
                max_level_dict[c] = 1
        # --- 4. 关键逻辑：修正和收紧约束范围 --- 确保了生成的份额组合最终能加起来等于1
        # determine maximum allowed values修正每列的最大值：一列的最大值不能超过 1 减去所有其他列的最小值之和。
        for c in column_names:
        ...
        # determine minimum allowed values修正每列的最小值：一列的最小值不能低于 1 减去所有其他列的最大值之和。
        for c in column_names:
            other_columns = sorted(list(set(column_names).difference({c})))
            min_level_dict[c] = min(1, max(min_level_dict[c], 1 - np.sum([max_level_dict[c] for c in other_columns])))
        # --- 5. 生成份额组合 ---
        # calculate span of values (max-min)计算每列的“跨度”（span），即修正后的最大值与最小值之差。
        span_dict = dict()
        for c in column_names:
            span_dict[c] = max_level_dict[c] - min_level_dict[c]
        # 按跨度从小到大对列名进行排序。
        # create list of column names sorted by span, smallest to biggest这是一个优化策略：先处理约束最紧（跨度最小）的列，将跨度最大的列留到最后计算。
        column_names_sorted_by_span = sorted(span_dict, key=span_dict.__getitem__)

        # generate a range of shares for the first n-1 columns为前 n-1 个列生成一系列可能的份额值
        scalars_only = True
        members = dict()
        for c in column_names_sorted_by_span[:-1]:
            if max_level_dict[c] > min_level_dict[c]:# 如果该列有浮动范围，使用 linspace 在其 min/max 之间生成 num_levels 个等间距的值。
                members[c] = linspace(min_level_dict[c], max_level_dict[c], num_levels)
                scalars_only = False #此时包含矩阵
            else:# 如果该列的 min 和 max 相等（即份额被锁定），则只使用这一个值。
        ...
        if scalars_only:
            ...
            members = pd.DataFrame.from_dict(members)
        # --- 6. 计算笛卡尔积并确定最后一列的值 --- 因为刚才只计算了前 n-1 列的份额值
        # generate cartesian product of first n-1 columns对前 n-1 列的所有可能份额值进行笛卡尔积，生成所有可能的组合。
        x = pd.DataFrame()
        for m in members:
            x = cartesian_prod(x, pd.DataFrame(members[m]))
        # 计算最后一列（跨度最大的列）的值，使其等于 1 减去其他列的和。
        # calculate values for the last column, honoring it's upper and lower limits
        last = column_names_sorted_by_span[-1]#用 np.maximum 和 np.minimum 确保这个计算出的值不会超出它自身的 min/max 约束。
        ...
        # remove rows that don't add up to 1 and get rid of join column ('_')过滤掉那些由于浮点数精度问题导致总和不完全等于1的行。
        ...
        if verbose:# 如果 verbose 为 True，则打印最终的分配方案。
...
def distribute_by_attribute(obj_list, value, weight_by, distribute_to):
    """作用是按照对象某个属性的权重，将一个总值分配到一组对象的另一个属性上
...
def weighted_value(objects, weight_attribute, attribute, attribute_args=None):
    ...
    weighted_sum = 0
    total = 0
    equal_weight = all([o.__getattribute__(weight_attribute) == 0 for o in objects])#检查权重是否全部为零
    for o in objects:
        if equal_weight:
            weight = 1
        else:
        ...
        total += weight
        if callable(o.__getattribute__(attribute)):#判断要加权的属性是否可调用（函数或方法）
            weighted_sum = weighted_sum + o.__getattribute__(attribute)(attribute_args) * weight
        else:#直接获取属性值，乘以权重，累加到加权和中
...
def generate_constrained_nearby_shares(columns, combo, half_range_frac, num_steps, min_constraints, max_constraints,
                                       producer_consumer_iteration_num=0, verbose=False):
    """
    ...
    #列重排序 ：函数将包含 NO_ALT（通常是份额固定的燃油车）的列放在前面，将 ALT（份额灵活的替代动力车）的列放在后面；因为最后一列的值是通过 1 - sum(其他列) 计算得出的，把最灵活的 ALT 列放在最后，可以最大程度地避免因为计算导致前面更严格的 NO_ALT 约束被意外违反。
    ...
    if all([min_constraints[c] == max_constraints[c] or
        ...
        for i in range(0, len(columns) - 1):#遍历除最后一列之外的所有列，为每一列生成一组候选份额值
            shares = np.array([])

            k = columns[i]
            val = combo[k]#首先从 combo 中取出中心值 val

            min_k = min_constraints[k]
            max_k = max_constraints[k]
            # 在大迭代后期，为NO_ALT引入弹性 (使用更健壮的判断方式)
            if producer_consumer_iteration_num > 1 and 'NO_ALT' in k.split('.') and min_k == max_k:
                flexibility_factor = 0.03 
                base_share = min_k
                min_k = base_share * (1 - flexibility_factor)
                max_k = base_share * (1 + flexibility_factor)
            
            min_val = ASTM_round(np.maximum(min_k, val - half_range_frac), machine_resolution)
            max_val = ASTM_round(np.minimum(max_k, val + half_range_frac), machine_resolution)
            ...
            if min_val == max_val:#NO_ALT 列的份额值是锁定的，执行这块代码
                shares = np.append(shares, ASTM_round(val, machine_resolution))
            else:
                if omega_globals.options.producer_compliance_search_multipoint:#如果为 True，则生产者合规性搜索会同时从合规点上方和下方逼近（如果可能）
                    # create new share spread and include previous value 使用 np.linspace 在这个小窗口内生成 num_steps 个均匀分布的份额值
        ...
        dfx = pd.DataFrame()
        for df in dfs:#将 dfs 列表中为前 N-1 列生成的所有候选值进行组合
            dfx = cartesian_prod(dfx, df)

        dfx = dfx[dfx.sum(axis=1).values <= 1].copy() #加了.copy()明确是一个全新的副本； 过滤掉那些不合逻辑的组合（即前 N-1 列的份额加起来已经超过100%的组合）
        dfx.loc[:, columns[-1]] = np.maximum(0.0, 1 - dfx.sum(axis=1).values)#最后一列的值被设定为 1 - (其他所有列的和)
