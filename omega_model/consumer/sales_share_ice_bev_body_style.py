        ...
        for pass_num in [0, 1]:
            for market_class_id in child_market_classes:
                if pass_num == 0:
                    ...
                    '''# --- 优化点：为中国市场引入新的效用维度 ---
                    # 1. 获取车型的基础信息 (这可能需要从 producer_decision 或 market_class_data 中获取更详细的属性)
                    # 假设我们可以获取到这些属性
                    tech_score = producer_decision.get('average_tech_score_%s' % market_class_id, 0.5) # 0-1分，越高越智能
                    is_domestic_brand = producer_decision.get('is_domestic_brand_%s' % market_class_id, False)
                    license_plate_advantage_dollars = producer_decision.get('license_plate_advantage_%s' % market_class_id, 0) # 虚拟补贴金额
                    
                    # 2. 将非经济因素转化为等效的“感知成本调整”
                    # 智能化水平调整：得分越高，感知成本越低。用一个系数来量化其重要性。
                    tech_cost_adjustment = (1 - tech_score) * 5000 # 假设满分智能化的吸引力等同于便宜5000美元
                    
                    # 品牌/国潮调整：如果是国产品牌，给予一定的成本减免
                    brand_cost_adjustment = -3000 if is_domestic_brand else 0
                    
                    # 3. 将所有成本项相加，得到最终的“综合感知成本”
                    # 注意：牌照优势和调整项需要分摊到车辆生命周期，与广义成本的计算方式保持一致
                    # 为简化演示，我们直接在总成本上加减
                    annualization_factor = 0.25 # 假设一个简化年金系数
                    perceived_cost_adjustment = (tech_cost_adjustment + brand_cost_adjustment - license_plate_advantage_dollars) * annualization_factor / 12000 # 转换为$/mi
                    
                    total_cost_w_fuel_per_PMT = total_cost_w_fuel_per_PMT + perceived_cost_adjustment
                    #修改结束'''
