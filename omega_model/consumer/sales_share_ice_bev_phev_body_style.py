...
class SalesShare(OMEGABase, SalesShareBase):
    ...
    def calc_consumer_generalized_cost(calendar_year, market_class_data, market_class_id, producer_decision):
        ...
        price_amortization_period = float(gcam_data_cy['price_amortization_period'])#价格摊销期
        discount_rate = gcam_data_cy['discount_rate']#贴现率
        annualization_factor = discount_rate + discount_rate / (
                ((1 + discount_rate) ** price_amortization_period) - 1)#计算年化因子。年化因子用于将一次性的购车成本转换为等效的年度成本
        ...
        recharge_efficiency = OnroadFuel.get_fuel_attribute(calendar_year, 'US electricity',
                                                            'refuel_efficiency')
        annual_o_m_costs = gcam_data_cy['o_m_costs']#从GCAM参数中获取年度的运维成本
        ...
        fuel_cost_per_VMT += fuel_cost * average_co2e_gpmi / carbon_intensity_gasoline / refuel_efficiency#电力和汽油成本

        # consumer_generalized_cost_dollars = total_capital_costs
        annualized_capital_costs = annualization_factor * total_capital_costs#将总资本成本（购车价）乘以年化因子，得到年化的资本成本
        annual_VMT = float(gcam_data_cy['annual_vmt'])
        total_non_fuel_costs_per_VMT = (annualized_capital_costs + annual_o_m_costs) / annual_VMT#计算每英里的非燃料成本
    ...
    def calc_shares_gcam(producer_decision, market_class_data, calendar_year,
        ...
        基于著名的 Logit 模型 来模拟消费者的购车决策。当面对同一类别（如 sedan_wagon）下的不同技术选项（如 ICE 和 BEV）时，该函数会根据每种选项的“广义成本”（包括购车价、使用成本等）来计算消费者对它们各自的偏好程度，最终得出它们的市场份额
        Args:
            producer_decision (Series): selected producer compliance option with
                'average_ALT_retail_fuel_price_dollars_per_unit_MC',
                'average_ALT_onroad_direct_co2e_gpmi_MC', 'average_ALT_onroad_direct_kwh_pmi_MC' attributes,
        ...
        sales_share_denominator = 0#初始化 Logit 模型份额公式的分母为0
        sales_share_numerator = dict()#初始化一个字典，用来存放每个子类别份额公式的分子
            ...
            for market_class_id in child_market_classes:
                if pass_num == 0:
                    ...
                    sales_share_numerator[market_class_id] = gcam_data_cy['share_weight'] * (
                            total_cost_w_fuel_per_PMT ** logit_exponent_mu)#这行是 Logit 公式的核心。它计算了该车型的“吸引力”或效用，公式为 权重 * (成本 ^ 指数)。成本越高，这个值越小，吸引力越低

                    sales_share_denominator += sales_share_numerator[market_class_id]#将当前车型的吸引力累加到总分母上。在遍历完所有子类别后，分母就等于所有选项的吸引力之和
                    ...
                    demanded_share_raw = sales_share_numerator[market_class_id] / sales_share_denominator#计算原始的、未受约束的相对市场份额，公式为 单个选项的吸引力 / 所有选项吸引力之和

                    # constrain relative (and by extension, absolute) shares RV
                    share_name = market_class_id.replace(parent_market_class + '.', '')
                    demanded_share = np.minimum(np.maximum(min_constraints[share_name], demanded_share_raw),
                                                max_constraints[share_name])#这是一个“钳位”操作。np.maximum 确保份额不低于最小值，np.minimum 确保份额不高于最大值。这样，demanded_share 就是被强制约束在 [min, max] 区间内的份额

                    if all(demanded_share == max_constraints[share_name]):
                        market_class_data['consumer_constrained_%s' % parent_market_class] = TRUE #这表示市场需求已经饱和，即使再降价也无法增加份额，因为被强制设定为了最大值

                    parent_share = market_class_data['consumer_abs_share_frac_%s' % parent_market_class].values

                    demanded_absolute_share = demanded_share * parent_share#用受约束后的相对份额乘以父级份额，得到该子类别在整个市场中的绝对份额

                    market_class_data['consumer_share_frac_%s' % market_class_id] = demanded_share #存入这个类别的相对市场份额
        ...
        reconciliation_df = pd.DataFrame()#份额再平衡 (Reconciliation)：在第二遍循环结束后，由于每个子类别的份额都被独立地施加了约束，它们的总和很可能不再是1。例如，一个份额被调低了，另一个没变，总和就会小于1。这个阶段的目标就是通过迭代微调，使它们的总和重新回到1。
        abs_share_columns = []
        share_columns = []
        share_names = []
        for market_class_id in child_market_classes:
        ...
        for N in range(len(share_columns), 0, -1):
            reconciliation_df['change_needed'] = 1 - reconciliation_df[share_columns].sum(axis=1)#计算当前所有份额的总和与目标值1之间的差额。如果总和是0.95，change_needed 就是+0.05；如果总和是1.02，change_needed 就是-0.02
            #这个函数会将 change_needed 的值平均分配给 N 个类别（change_needed / N），然后加到（或减去）当前的份额上。重要的是，这个调整过程同样会受到最小/最大份额的约束，确保调整后的值不会越界
    ...
    def calc_shares(calendar_year, compliance_id, producer_decision, market_class_data, mc_parent, mc_pair):
        ...
        if all([SalesShare.gcam_supports_market_class(mc) for mc in mc_pair]):#检查消费者模型（GCAM）是否有计算 mc_pair 中所有类别份额所需的参数。只有参数齐全，才能进行后续计算
            if len(mc_pair) > 1:
                ...
                market_class_data['average_ALT_cross_subsidized_price_%s' % only_child] = \
                    producer_decision['average_ALT_new_vehicle_mfr_cost_%s' % only_child]#这里直接将车辆的“交叉补贴后价格”和“修正后的交叉补贴价格”设置为其原始的“制造成本”
