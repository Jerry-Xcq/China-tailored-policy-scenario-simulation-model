    ...
    def get_cloud(vehicle):
        ...
        vehicle_rlhp20 = \
            ASTM_round(calc_roadload_hp(vehicle.base_year_target_coef_a, vehicle.base_year_target_coef_b,
                             vehicle.base_year_target_coef_c, 20), sweep_precision)#分别计算车辆在20mph和60mph下的路负荷马力（roadload hp）
        ...
        for ccc in cost_curve_classes:
            ...
            if vehicle.prior_redesign_year == vehicle.model_year:
                # update charge depleting range if it's a redesign year 如果是重新设计年份，则更新续航里程等参数
                ...
                for footprint_ft2 in vehicle_footprints:
                    for rlhp20 in rlhp20s:
                        for rlhp60 in rlhp60s:
                            ...
                            converged = False
                            while not converged: #迭代收敛地确定车辆的动力系统参数
                                # rated hp sizing --------------------------------------------------------------- #
                                ...
                                if vehicle.powertrain_type == 'BEV' or vehicle.powertrain_type == 'PHEV':
                                    ...
                                    cloud_point = vehicle.calc_battery_sizing_onroad_direct_kWh_per_mile(cloud_point) #为下面使用 单位里程耗电因子 做铺垫
                                    ...
                                    if vehicle.powertrain_type == 'PHEV' and \
                                        ...
                                        # override battery size by kWh 直接强制指定电池容量
                                        battery_kwh = omega_globals.options.phev_battery_kwh
                                    else:
                                        # BEVs and range-sized PHEVs  BEV类型和使用续航计算的PHEV类型用这个else  电池总容量 = 纯电续航里程 × 单位里程能耗 ÷ 可用容量比例
                                ...
                                prior_battery_kwh = battery_kwh #参数之间相互影响，必须多次迭代，直到各项参数变化很小，才认为找到了合理的车辆配置
                                #动力系统质量、额定马力、（如果是BEV/PHEV还包括电池容量）的相对变化都小于convergence_tolerance（通常是一个很小的数，比如0.01，表示变化小于1%）
                                # ------------------------------------------------------------------------------------#
        ...
        for idx, ct in enumerate(glider_cost_terms):
            ...
            cost_cloud.loc[:, ct] = glider_costs[idx] #要注意这里的赋值方式
            '''cost_cloud = cost_cloud.copy()  # 确保是副本
            cost_cloud[ct] = glider_costs[idx]'''
        ...
        # clear all tech flags 清空车辆的技术标志，避免影响后续计算
        for tf in CostCloud.tech_flags:
            vehicle.__setattr__(tf, 0)
