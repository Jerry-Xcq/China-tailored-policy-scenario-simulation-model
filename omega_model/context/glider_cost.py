...
class GliderCost(OMEGABase):
    ...
    **Loads and provides access to glider cost data, provides methods to calculate glider costs.**
    ...
    def get_markups_and_learning(vehicle):
        ...
        Args:根据传入的 vehicle（车辆对象），获取与该车辆相关的加价系数（markup）和学习因子（learning factor）等参数，用于后续的成本计算。
            vehicle:
        ...
        cumulative_sales = abs(sales_scaler * (vehicle.model_year - learning_start))#计算累计销量（cumulative_sales），再根据学习率公式计算学习因子（learning_factor）。如果车辆年份早于学习起始年份，则对学习因子取倒数
